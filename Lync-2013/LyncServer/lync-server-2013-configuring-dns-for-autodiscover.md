...
DNS レコードは、追加のサブジェクト代替名 (SAN) で新しい証明書を作成する機能に基づいて、(ホスト) レコードまたは CNAME レコードのいずれかにすることができます。 Lyncdiscover を使用して、新しい外部 (公開) 証明書を要求して展開できない場合。\<domain name\> SAN では、HTTP/TCP ポート80を使用する手順について説明します。 次の手順では、内部と外部の DNS レコードを作成する方法について説明します。
