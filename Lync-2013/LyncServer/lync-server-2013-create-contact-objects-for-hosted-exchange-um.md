...
2.  New-CsExUmContact コマンドレットを実行して、展開に必要な連絡先オブジェクトを作成します。 たとえば、次のように実行して AA と SA の連絡先オブジェクトを作成します。
    ...
    これらの例では、次のパラメーターを設定します。
      ...
      - **SipAddress** 連絡先オブジェクトの SIP アドレスを指定します。 これは、Active Directory ドメインサービスでユーザーまたは連絡先オブジェクトを構成するためにまだ使用されていないアドレスである必要があります。 この値は、前の例のように "sip:" の形式で指定する必要があり \<*SIP address*\> ます。
