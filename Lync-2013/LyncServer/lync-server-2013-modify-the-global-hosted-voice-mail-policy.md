...
*グローバル* にホストされるボイスメールポリシーは、Lync Server 2013 と共にインストールされます。 必要に応じて変更できますが、名前の変更や削除はできません。 グローバルポリシーを変更するには、Set-CsHostedVoicemailPolicy コマンドレットを使用して、特定の展開用にパラメーターを適切な値に設定します。
...
2.  環境のグローバルポリシーパラメーターを設定するには、Set-CsHostedVoicemailPolicy コマンドレットを実行します。 たとえば、以下を実行します。
