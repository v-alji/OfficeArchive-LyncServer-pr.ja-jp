...
自動的な消去に加えて、2つの新しいコマンドレット (Invoke-CsCdrDatabasePurge と Invoke-CsQoEDatbasePurge が Microsoft Lync Server 2013 に追加されています。これらのコマンドレットを使用すると、管理者はいつでも CDR と QoE データベースからレコードを手動で削除できます。 たとえば、CDR データベースから10日以上経過しているすべてのレコードを手動で消去するには、次のようなコマンドを使用できます。
