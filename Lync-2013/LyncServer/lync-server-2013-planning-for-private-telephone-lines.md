...
専用の電話回線は、Lync Server の展開用にのみ構成する必要があります。 Lync Server と Office 2007 Communications server 2007 R2 サーバーの両方が含まれている展開では、以前のバージョンのユーザーがプライベート電話回線に電話をかけようとしたときに、サーバーがプライベート電話回線で逆引き番号参照を実行できないため、通話のルーティングが失敗します。
