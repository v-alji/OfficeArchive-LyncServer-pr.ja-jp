...
トポロジが展開された後にトポロジを操作する前に、次の Active Directory と DNS レコードが作成されていることを確認してください (具体的な機能の設定が必要な場合)。
  ...
  - \_Sipinternaltls tcp の形式でクライアントの自動ログオンを使用する予定がある場合は、各 SIP ドメインに対して DNS SRV レコードが存在します。 \_ \<SIP domain\> クライアントに手動構成を使用する場合は、このレコードは必要ありません。
